...
//    }
    ...
    {
        if (!isfinite(adc_data.cap_v) || !isfinite(cap_vol_max_limit))//检查输入数据的有效性
        {
        pid_bat_power.OutCurrent = 0;//无效数据不输出
        return;
        }
    ...
    else if(adc_data.cap_v > cap_vol_max_limit - 2.0f && pid_bat_power.OutCurrent > 0)
    {
        ...
        float difference = cap_vol_max_limit - adc_data.cap_v;//当前电容电压与电容最大电压的差值
        if(difference > charge_tolerance)//如果差值大于容差，则限制输出电流（设置一个阈值防止数值跳变影响稳定）
        {
...
void cap_pid_powerbuffer()
{
    ...
    //低功率缓冲时快速恢复
    if(pid_powerbuffer.ActualBuffer < 42)
    {
		//* 防止底盘功率超限
